...
Class db extends connect{
    ...
    public function set_fields( $items, $delimiter = "," ) {// возвращает строку значений ид в формате `0`=:0,`1`=:1 и т.д.
        $str = array();
    ...
    public function get_count( $where = array() )//возвращает общее количество
    {
    ...
    public function get_all($order = "id asc")//возвращает массив всех значений
    {
    ...
    public function get_one($where = [], $order = "id asc")
    {
        ...
        $sql .= " ORDER BY $order";// SELECT * FROM {$this->table_name} WHERE ? AND ?... ORDER BY id asc
        $stmt = $this->pdo->prepare($sql);
        $stmt->execute($where);
        $result = $stmt->fetch();
    ...
    public function createList($table_name){
